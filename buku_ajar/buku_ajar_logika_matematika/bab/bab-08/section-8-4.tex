...
\begin{enumerate}
    \item \textbf{Terima query:}\newline
    Identifikasi apa yang ingin dibuktikan, misalnya\newline
    $Kakek(\text{Hasan}, \text{Ali})$?
...
\begin{konsep}
Rantai penalaran di atas menunjukkan bahwa fakta-fakta baru ($Mortal(Marcus)$, $Tewas(Marcus, Erupsi, 79)$, $Legenda(Marcus)$) tidak tercantum secara eksplisit dalam KB, tetapi dapat \textbf{diturunkan} melalui inferensi logis. Hal ini menggambarkan keunggulan pendekatan basis pengetahuan: menurunkan kesimpulan yang tidak langsung tersirat dari fakta dan aturan yang ada.
\end{konsep}
