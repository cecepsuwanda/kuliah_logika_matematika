...
\begin{itemize}
    \item \textcolor{blue}{Kelompok 1}: $\{m_0, m_1, m_4, m_5, m_8, m_9, m_{12}, m_{13}\}$ --- 8 sel pada dua kolom kiri ($CD = 00$ dan $CD = 01$). Variabel $C = 0$ konstan, $D$ berubah $\Rightarrow$ suku: $C'$
    \item \textcolor{red}{Kelompok 2}: $\{m_0, m_2, m_4, m_6\}$ --- 4 sel pada dua baris atas, kolom $00$ dan $10$ (wrap-around). Variabel $B = 0$ dan $D = 0$ konstan $\Rightarrow$ suku: $B'D'$
    \item \textcolor{green!50!black}{Kelompok 3}: $\{m_{12}, m_{14}\}$ --- 2 sel pada baris $11$, kolom $00$ dan $10$ (wrap-around). Variabel $A = 1$, $B = 1$, $D = 0$ konstan $\Rightarrow$ suku: $ABD'$
\end{itemize}
...
Perhatikan bahwa $C'D' + CD = C \odot D$ (XNOR), yaitu fungsi yang bernilai 1 ketika $C$ dan $D$ memiliki nilai yang sama. Hal ini menunjukkan bahwa K-Map tidak hanya menyederhanakan ekspresi, tetapi juga dapat mengungkap struktur logis yang tidak segera terlihat dari bentuk awal.
\end{contoh}
