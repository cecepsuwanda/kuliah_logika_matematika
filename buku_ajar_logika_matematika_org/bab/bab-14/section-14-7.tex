...
Verifikasi formal bukan sekadar latihan akademis --- ia memiliki implikasi nyata dan kritis dalam pengembangan perangkat lunak, terutama untuk sistem-sistem yang memerlukan tingkat keandalan tertinggi. Section ini membahas mengapa verifikasi formal menjadi semakin penting dan bagaimana konsep-konsep yang telah dipelajari diterapkan dalam praktik industri \cite{rosen2019}.
...
Sebagaimana dinyatakan oleh Edsger Dijkstra: ``\textit{Program testing can be used to show the presence of bugs, but never to show their absence.}'' (Pengujian program dapat menunjukkan keberadaan bug, tetapi tidak pernah membuktikan ketiadaannya.) Pernyataan ini menggambarkan keterbatasan fundamental testing dan motivasi utama verifikasi formal.

\subsection{Mission-Critical Systems}

Verifikasi formal menjadi sangat kritis pada \logic{mission-critical systems} --- sistem di mana kegagalan dapat mengakibatkan kerugian jiwa, finansial, atau lingkungan yang besar:

\begin{itemize}
    \item \textbf{Aviasi}: Perangkat lunak kontrol penerbangan pada pesawat komersial (misalnya Airbus A380) melalui proses verifikasi formal untuk memastikan kebenaran algoritmanya.
    \item \textbf{Medis}: Perangkat lunak pada alat pacu jantung dan pompa infus harus diverifikasi karena kesalahan dapat mengancam nyawa pasien.
    \item \textbf{Antariksa}: Kegagalan perangkat lunak pada roket Ariane 5 (1996) yang disebabkan oleh overflow integer mengakibatkan kerugian ratusan juta dolar, menunjukkan pentingnya verifikasi formal.
...
\begin{enumerate}
    ...
    \item \textbf{Theorem Provers}: Alat seperti Coq, Isabelle, dan Lean memungkinkan penulisan bukti formal yang diperiksa oleh komputer.
...
\begin{konsep}
Verifikasi program merupakan aplikasi puncak dari seluruh konsep logika matematika yang telah dipelajari. Kemampuan berpikir logis secara formal --- mulai dari memahami proposisi hingga menyusun bukti --- pada akhirnya diterapkan untuk menjamin bahwa perangkat lunak yang kita buat bekerja sesuai spesifikasi, bukan berdasarkan asumsi atau harapan, melainkan berdasarkan \textbf{bukti matematis}.
\end{konsep}
