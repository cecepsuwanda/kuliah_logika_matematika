...
set "SOURCE_DIR=%~dp0"
set "MAIN_TEX=tugas1.tex"

if not exist "%OUTPUT_DIR%" mkdir "%OUTPUT_DIR%"

echo ============================================================
echo Compiling Tugas Document ^(%MAIN_TEX%^)
echo ============================================================
...
echo Running Stage 2: pdflatex ^(referensi silang / LastPage^)...
pdflatex -interaction=nonstopmode -halt-on-error -output-directory="%OUTPUT_DIR%" "%MAIN_TEX%"
if errorlevel 1 goto :failed
...
for %%F in ("%MAIN_TEX%") do set "MAIN_BASE=%%~nF"

echo.
echo Operation Completed.
echo Final PDF: %OUTPUT_DIR%\%MAIN_BASE%.pdf
goto :end
