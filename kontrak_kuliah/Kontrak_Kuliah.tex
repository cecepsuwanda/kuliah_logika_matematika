	...
	\begin{frame}{Kontrak Perkuliahan}
		\begin{block}{Penilaian}
			\begin{itemize}
				...
				\item \textbf{Quiz:} 10\% dari nilai akhir. (edLink)
				\item \textbf{Tugas:} 10\% dari nilai akhir.
			\end{itemize}
		\end{block}
	\end{frame}
	...
	\begin{frame}{Kontrak Perkuliahan}		
		\begin{block}{Aturan Kuliah}
			\begin{itemize}
				...
				\item Ketua Kelas membuat group WA perkuliahan, nama group ``Logika Matematika 2025''. 
...
% \end{frame}
	...
	\begin{frame}{Materi Kuliah}
		\begin{itemize}
			\item \textbf{Logika Proposisional:} Konsep dasar, tabel kebenaran, inferensi.
			\item \textbf{Logika Predikat:} Fungsi proposisional, kuantor universal ($\forall$), eksistensial ($\exists$).
			\item \textbf{Pemodelan Masalah:} Pemodelan berbasis logika predikat.
			\item \textbf{Aljabar Boolean:} Prinsip, hukum dasar, Peta Karnaugh (K-Map).
			\item \textbf{Sistem Digital:} Rangkaian gerbang logika dasar.
			\item \textbf{Pembuktian:} Pembuktian langsung, tidak langsung (kontradiksi, kontraposisi).
			\item \textbf{Induksi Matematika:} Argumen matematis formal dan validasi algoritma dasar.
		\end{itemize}
	\end{frame}
